class Solution:
    def maxProfit(self, prices: List[int], fee: int) -> int:
         return self.BestTime(prices,0,True,{},fee) # this is our calling function
    
    def BestTime(self,prices,currentDay , canBuy, memo,fee):
        '''
        input parameters
        ...
        if canBuy:
            ideal = self.BestTime(prices,currentDay + 1,canBuy, memo,fee) # we sit ideal at that day
            buy = -prices[currentDay] + self.BestTime(prices,currentDay + 1,False, memo,fee) # we buy the stock at that day
            memo[currentKey] = max(ideal,buy)
            return memo[currentKey]
        else:
            ideal = self.BestTime(prices,currentDay + 1,canBuy,  memo,fee) # we sit ideal at that day
            sell = -fee + prices[currentDay] + self.BestTime(prices,currentDay + 1,True,  memo,fee) # we sell the stock at that day
